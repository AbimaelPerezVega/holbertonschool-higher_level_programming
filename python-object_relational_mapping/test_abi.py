...
(5, 'Nevada')"""
    ...
    def test_no_extra_newline(self):
        ...
        self.assertEqual(len(output_lines), 5, "Extra newline detected in output!")
    ...
    def test_empty_table(self):
        ...
        command = [f"./{self.script_name}", self.db_user, self.db_password, "hbtn0e0usa_empty"]
        result = subprocess.run(command, capture_output=True, text=True)
    ...
    def test_special_characters(self):
        ...
        command = [f"./{self.script_name}", self.db_user, self.db_password, "hbtn0e0usa_special"]
        result = subprocess.run(command, capture_output=True, text=True)
    ...
    def test_large_number_of_rows(self):
        ...
        command = [f"./{self.script_name}", self.db_user, self.db_password, "hbtn0e0usa_large"]
        result = subprocess.run(command, capture_output=True, text=True)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(Fore.RED + f"FileNotFoundError: {script_name} not found." + Style.RESET_ALL)
